...
class AuthController extends CI_Controller
{
  ...
  public function index()
  {
    ...
    $this->load->view('dashboard/gate/login');
  }
  ...
  public function login_ajax()
  {
    ...
    if ($action['status'] == 'failed') {
      ...
      if ($check) {
        if (hash('md5',$post['password']) == $check['password']) {
          $this->AuthModel->updateLogin($check['id_user'], 1);
          $session = array(
            'logged' => TRUE,
            'username' => $check['id_user'],
            'role' => $check['role'],
        ...
        } else if ($post['password'] != $check['password']) {
          $response = array(
            'status' => 'failed',
  ...
  public function logout()
  {
    ...
    $update = $this->AuthModel->updateLogin($id, "0");
